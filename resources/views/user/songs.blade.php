...
<div class="flex">
        <iframe class="mb-4" style="border-radius:12px" src="https://open.spotify.com/embed/track/{{ $item->id }}?utm_source=generator" width="100%" height="152" frameBorder="0" allowfullscreen="" allow="autoplay; clipboard-write; encrypted-media; fullscreen; picture-in-picture" loading="lazy"></iframe>
            {{-- <form method="POST" action="{{ route('add.songs',$item->id) }}">
                @csrf --}}
        <button type="button" data-modal-target="playlist-modal-{{ $item->id }}" data-modal-toggle="playlist-modal-{{ $item->id }}" class="self-center ml-4 h-1/2 text-gray-900 bg-white hover:bg-gray-100 border border-gray-200 focus:ring-4 focus:outline-none focus:ring-gray-100 font-medium rounded-lg text-sm px-5 py-2.5 text-center inline-flex items-center dark:focus:ring-gray-600 dark:bg-gray-800 dark:border-gray-700 dark:text-white dark:hover:bg-gray-700">
        <svg aria-hidden="true" class="w-4 h-4 me-2" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13.828 10.172a4 4 0 00-5.656 0l-4 4a4 4 0 105.656 5.656l1.102-1.101m-.758-4.899a4 4 0 005.656 0l4-4a4 4 0 00-5.656-5.656l-1.1 1.1"></path></svg>
        Add to Playlist
        </button>

        <!-- Main modal -->
        <div id="playlist-modal-{{ $item->id }}" tabindex="-1" aria-hidden="true" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
                        ...
                        <ul class="my-4 space-y-3">
                            ...
                            <li>
                                <button type="submit" class="flex items-center p-3 text-base font-bold text-gray-900 rounded-lg bg-gray-50 hover:bg-gray-100 group hover:shadow dark:bg-gray-600 dark:hover:bg-gray-500 dark:text-white">
                                    ...
                                    <span class="flex-1 ms-3 whitespace-nowrap">{{ $playlist->name }}</span>
                                </button>
                            </li>
                            </form>
                            @endforeach
